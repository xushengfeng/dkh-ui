...
type FilteredKeys<T> = {
    [K in keyof T as HasUnderscoreKey<T[K]> extends true ? K : never]: Omit<T[K], "_">;
};

type Ex<T, K> = K extends keyof T ? T[K] : Ex<UnionToIntersection<MergedValues<T>>, K>;
...
type SetViewValue<T> = { [key in keyof T]: "_" extends keyof T[key] ? T[key]["_"] : T[key] };
